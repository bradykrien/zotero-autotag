...
- `src/zotero_autotag/vocab_generator.py` — Ollama batch sampling + consolidation; `load_proposals_with_descriptions()`
- `src/zotero_autotag/tag_assigner.py` — sentence-transformers cosine similarity; business logic (preserve tbr / overwrite / add-only); accepts descriptions dict
- `src/zotero_autotag/description_generator.py` — Mistral-powered tag description generation; `DescriptionGenerator` class; `update_descriptions_in_yaml()`
...
- `scripts/dashboard.py` — Phase 3+: Streamlit dashboard; tabs: Overview, Item Types, Tags, PDF Coverage, Tag Assignments, Browse
- `scripts/generate_vocab.py` — Phase 4: propose vocabulary → `data/vocab_proposals.yaml`
- `scripts/generate_descriptions.py` — Phase 4.5: generate tag descriptions → updates `descriptions:` block in `vocab_proposals.yaml`; `--refresh` to regenerate all
- `scripts/assign_tags.py` — Phase 5: dry-run (default) or `--apply` to write to Zotero; `--limit N` for testing
- `data/vocab_proposals.yaml` — git-tracked; 39 approved tags + descriptions; human-reviewed before Phase 5
...
- Tag descriptions: embed `"tag: description"` instead of bare tag names for better semantic discrimination; generated by `generate_descriptions.py`, stored in `vocab_proposals.yaml`
- Embedding model: `all-mpnet-base-v2` (better semantic discrimination than MiniLM, especially for overlapping noun phrases like "digital humanities" vs "environmental humanities")
- Similarity threshold: `0.38` (tuned); max tags per item: `3`; date horizon: `30` days
- `tbr` tag: items with tbr are processed normally; tbr is preserved in `final_tags` but is never added or removed
